...
if __name__ == '__main__':
  ...
  for i in range(num_images):
      ...
      tensor2 = np.squeeze(tensor, axis=0)
      t3 = np.rollaxis(tensor2,0,3)
      image = (t3 * 255).round().astype(np.uint8)
      #for line in lines: 
      abc = Image.fromarray(image, 'RGB')
      abc.save(os.path.join(path, "%s.%s" % (lines[i],'jpg')))
      #abc.save(path+str(lines[i])+'.jpg')
...
'''
      ...
      #print(img.shape)
      #img2 = img.squeeze(1)
      #print(img.shape) ## 1 st 
      #img2 = transforms.ToPILImage()(img)
      tensor  = img.cpu().numpy() 
      tensor2 = np.squeeze(tensor, axis=0)
      #print(tensor2)
      t3 = np.rollaxis(tensor2,0,3)
      #img = t3.astype(np.uint8)
      image = (t3 * 255).round().astype(np.uint8)
      #print(image)
      abc = Image.fromarray(image, 'RGB')
      print(data)
      ...
      #abc = Image.fromarray(t3.astype('uint8'), 'RGB')
      #print(type(abc))
      #print(abc.size) ## 3 
      #abc.save('abc.jpg')
      ...
      #abc.save("file.jpeg")
      #print(img2.size)
      #torchvision.utils.save_image(img2,'test.jpg')
      #omg = img2.save('file.jpeg')
      #import torch
